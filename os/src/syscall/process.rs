...
use crate::{
    ...
    task::{
        ...
        suspend_current_and_run_next, copy_to_user, range_map, range_unmap, TaskControlBlock,
    },
    timer::get_time_us,
};
...
pub fn sys_spawn(path: *const u8) -> isize {
    ...
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        ...
        let new_pid = task.pid.0 as isize;
        let parent_task = current_task().unwrap();
        parent_task.inner_exclusive_access().children.push(task.clone());
        task.inner_exclusive_access().parent = Some(Arc::downgrade(&parent_task));
        add_task(task);
        new_pid
    } else {
        -1
